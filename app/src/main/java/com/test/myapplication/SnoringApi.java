...
public class SnoringApi extends DetectionApi {
	...
	String TAG = "SnoreRef_SnoringApi";
	...
	public int isSnoring(byte[] audioBytes) {
		...
		Log.d(TAG,"res " + Arrays.toString(res));
	...
	public void cal_threshold() {
		...
		// for(int i=0; i<this.ZCR.length; i++)
		// Log.d(TAG,this.ZCR[i]);
		// Log.d(TAG,this.getThreshold_E());
		// Log.d(TAG,this.getThreshold_ZCR());
	}
